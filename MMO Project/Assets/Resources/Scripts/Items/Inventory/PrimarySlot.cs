...
public class PrimarySlot : InventorySlot
{
    ...
    public override void SetHeldItem()
    {
        if (HeldItem != null)
        {
            ...
            PlayerPrefs.SetString("PrimaryID" + SlotNumber, ItemIDHeld);
            PlayerPrefs.SetInt("PrimaryCount" + SlotNumber, ItemsHeld);
            //Debug.Log("SlotID" + SlotNumber + " ID Saved: " + ItemIDHeld + ", Amount: " + ItemsHeld);
        }
        else
        {
            PlayerPrefs.SetString("PrimaryID" + SlotNumber, "None");
            PlayerPrefs.SetInt("PrimaryCount" + SlotNumber, 0);
        }
    ...
    {
        ItemIDHeld = PlayerPrefs.GetString("PrimaryID" + SlotNumber, "None");
        if (ItemIDHeld == "None")
        {
        ...
        {
            ItemsHeld = PlayerPrefs.GetInt("PrimaryCount" +
                "" + SlotNumber, 1);
            ...
            if (HeldItem != null)
            {
                Durability = PlayerPrefs.GetInt("PrimaryID_Durability" + SlotNumber, Durability);
                SetDurability(Durability);
    ...
    public override Item SwapItem(Item i = null)
    {
        ...
        if (WTemp != null)
        {
            ...
            if (WTemp.GetSlot() == Weapon.WeaponSlot.PrimarySecondary || WTemp.GetSlot() == Weapon.WeaponSlot.Primary || WTemp.GetSlot() == Weapon.WeaponSlot.All)
            {
                HeldItem = i;
                SetHeldItem();
                UpdateItemDisplay();
    ...
    {
        Durability = durable;
        PlayerPrefs.SetInt("PrimaryID_Durability" + SlotNumber, Durability);
        if (HeldItem != null)
